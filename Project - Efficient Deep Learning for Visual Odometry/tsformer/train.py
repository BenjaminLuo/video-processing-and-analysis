...
def train(model, train_loader, val_loader, criterion, optimizer, tensorboard_writer, args):
    ...
    for epoch in range(args["epoch_init"], epochs):
        ...
        if not epoch % 20:
            torch.save(state, os.path.join(checkpoint_path,
                       "checkpoint_e{}.pth".format(epoch)))
...
def get_optimizer(params, args):
    ...
    if args["checkpoint"] is not None:
        checkpoint = torch.load(os.path.join(
            args["checkpoint_path"], args["checkpoint"]), map_location=torch.device('cpu'))
...
def compute_loss(y_hat, y, criterion, args):
    if args["weighted_loss"] == None:
        ...
        # predict pose
        y_hat = torch.reshape(
            y_hat, (y_hat.shape[0], args["window_size"]-1, 6))
        ...
        loss_translation = criterion(
            estimated_translation, gt_translation.float())
...
if __name__ == "__main__":
    ...
    args = {
        "data_dir": "/Users/epheriami/Downloads/4B/Code/SYDE 673/P/dataset/sequences_jpg",
        "bsize": 4,  # batch size
        "val_split": 0.1,  # percentage to use as validation data
        "window_size": 4,  # number of frames in window
        ...
        "checkpoint_path": "checkpoints/Exp5",  # path to save checkpoint
    ...
    model_params = {
        "dim": 384,
        "image_size": (192//2, 640//2),  # (376, 1241)
        "patch_size": 16,
        # ['divided_space_time', 'space_only','joint_space_time', 'time_only']
    ...
    print("Loading data...")
    dataset = KITTI(window_size=args["window_size"],
                    overlap=args["overlap"], transform=preprocess)
    ...
    train(model, train_loader, val_loader, criterion,
          optimizer, TensorBoardWriter, args)
